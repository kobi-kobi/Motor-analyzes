...
hold on
%plot max/min locations in graph
plot(T(1:length(UD)),max_locations*mean(UD),"x");
hold on
plot(T(1:length(UD)),min_locations*mean(UD),"o");
hold on
% plot smooth graph
plot(T(1:length(UD)),UD_WITHOUT_NOISE,"green");
ylim([min(UD) max(UD)]);
clear device
%hold on
%plot locations of min in graph and max
%plot(T(1:length(UD)),max_locations*200,'red')
%hold on
%plot(T(1:length(UD)),min_locations*200,'green')
%% fft maker
%Y=fft(UD);
%Fs=2000;
